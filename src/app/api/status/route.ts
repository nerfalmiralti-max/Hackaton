...
import { authStatus, requireApplicationAuth } from "@/lib/server/auth-guard";
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ configured: Boolean(process.env.OPENAI_API_KEY?.trim()), schoolConnected: Boolean(await schoolStoreId()), model: modelName(), files: state.files, development: process.env.NODE_ENV === "development", auth: authStatus() }, { headers: { "Cache-Control": "no-store" } });
