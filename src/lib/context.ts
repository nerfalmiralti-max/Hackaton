...
export type WebMode = "off" | "manual" | "automatic-fallback";

export function buildInstructions(request: ChatRequest, strictSchool: boolean, webMode: WebMode = request.web ? "manual" : "off") {
...
${webMode === "manual" ? "The user explicitly enabled web search for this turn. Use it only if needed; cite native web sources." : webMode === "automatic-fallback" ? "The available school and uploaded materials were already searched and did not provide sufficient support for this question. Web search is authorized for this fallback turn. Search reliable official, primary, educational, university, or established reference sources when available. Clearly distinguish web-sourced information from NIS or uploaded materials, never imply that a web source came from the school knowledge base, never invent citations, and use native web citations. The application will add a short disclosure about this fallback." : "Web search is not authorized. Do not claim web access."}
