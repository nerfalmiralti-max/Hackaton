...
import { BookOpen, ChevronDown, Image as ImageIcon, Layers, Menu, MessageSquare, Moon, PanelRight, Plus, ShieldCheck, Sparkles, Sun, UserRound, Zap } from "lucide-react";
...
export function Workspace() {
  ...
  return <div className={`site-shell ${dark ? "dark" : ""}`}>
    ...
    <aside className={`sidebar ${menu ? "open" : ""}`} aria-label={t.menu}>
      ...
      <nav>{([{ key: "chat", icon: MessageSquare }, { key: "knowledge", icon: BookOpen }, { key: "profile", icon: UserRound }] as const).map(item => <button key={item.key} className={`nav-item ${view === item.key ? "selected" : ""}`} onClick={() => navigate(item.key)} aria-current={view === item.key ? "page" : undefined}><item.icon size={18}/><span>{t[item.key]}</span>{item.key === "knowledge" && app.status?.files.length ? <small>{app.status.files.length}</small> : null}</button>)}</nav>
