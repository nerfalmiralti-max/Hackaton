...
import type { KnowledgeFile, Recommendation, View } from "@/lib/types";
...
export function Workspace({ identity, onSignOut }: { identity?: AuthIdentity; onSignOut?: () => void }) {
  ...
  function askFile(file: KnowledgeFile) { app.setSelectedFile(file); navigate("chat"); void app.send(`${t.filePrompt} «${file.name}».`, undefined, { file }); }
    function askRecommendation(item: Recommendation) { void app.send(`${t.recommendationLearn}: ${item.title}\n\n${item.summary}\n\nОфициальный источник: ${item.sourceUrl}\nОтвечай только на основе этой публикации и не добавляй неподтверждённые факты.`); }
