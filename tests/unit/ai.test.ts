...
describe("citation provenance and school grounding", () => {
  ...
  it.each(["Unsupported statement.", "Unsupported statement. [L2]"])("withholds strict local answers without a supplied reference: %s", async text => {
    mocks.searchLocalKnowledge.mockResolvedValue([localHit]);
    upstream({ text });
    const events = await collect(chat({ sourceMode: "school" }));
    expect(textOf(events)).not.toContain("Unsupported");
    expect(doneOf(events)).toMatchObject({ sources: [], terms: [], needsFallback: true });
    expect(mocks.stream).toHaveBeenCalledOnce();
  });
  ...
  it.each(["en", "ru", "kk"] as const)("returns a localized fallback without a paid call when school stores are absent (%s)", async language => {
